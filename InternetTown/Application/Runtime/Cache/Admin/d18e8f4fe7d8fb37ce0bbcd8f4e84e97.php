...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            <?php if(is_array($adminusers)): foreach($adminusers as $key=>$adminuser): ?>CustomersData.Rows.push({
                        "id": "<?php echo ($adminuser["id"]); ?>",
                        "name": "<?php echo ($adminuser["username"]); ?>",
                        "group":"<?php echo ($adminuser["gid"]); ?>",
                        "created_time":"<?php echo (date('Y-m-d H:s:i',$adminuser["created_time"])); ?>",
                        "last_login":"<?php echo (date('Y-m-d H:s:i',$adminuser["last_login"])); ?>",
                        "login_time":"<?php echo ($adminuser["login_count"]); ?>"
        ...
        function itemclick(item) {
            if (item.icon === 'add') {
                $.ligerDialog.open({
                    ...
                    width:330,
                    title:"添加管理员",
                    url:"<?php echo U('System/adminUserAdd');?>",
                    slide:false,
                    buttons:[
            ...
            if (item.icon === 'delete') {
                ...
                $.ligerDialog.confirm('确认删除？', '删除', function (yes) {
                    if (yes)
                        $.post("<?php echo U('System/adminUserDel');?>", {uid:row.id}, function (res) {
                            if (!res.status)
