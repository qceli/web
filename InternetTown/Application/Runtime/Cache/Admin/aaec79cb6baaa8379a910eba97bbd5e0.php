...
<head>
    ...
    <script src="/Public/LigerUI/lib/ligerUI/js/plugins/ligerForm.js" type="text/javascript"></script>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            <?php if(is_array($ticket_order_info)): foreach($ticket_order_info as $key=>$ticket_order): ?>CustomersData.Rows.push({
                        "id":<?php echo ($ticket_order["id"]); ?>,
                        "trade_no":"<?php echo ($ticket_order["trade_no"]); ?>",
                        "t_name":"<?php echo ($ticket_order["t_name"]); ?>",
                        "count":<?php echo ($ticket_order["count"]); ?>,
                        "price":<?php echo ($ticket_order["price"]); ?>,
                        "customer":"<?php echo ($ticket_order["customer"]); ?>",
                        "phone":"<?php echo ($ticket_order["phone"]); ?>",
                        "use_time":"<?php echo (date("Y-m-d",$ticket_order["use_time"])); ?>",
                        "buy_time":"<?php echo (date("Y-m-d",$ticket_order["buy_time"])); ?>",
                        "status":"<?php echo ($ticket_order["status"]); ?>"
                    });<?php endforeach; endif; ?>

            CustomersData.Total = "<?php echo ($counts); ?>";
        ...
        function itemclick(item) {
            if (item.icon === 'search2') {
                ...
                $.ligerDialog.open({
                    ...
                    width: 300,
                    title: '查看详情',
                    url: "<?php echo U('Index/ticket_info');?>",
                    urlParms: {
                        tid: row.id
                    },
                    slide: false,
        ...
        function search_data() {
            ...
            $.post("<?php echo U('Index/search');?>", {phone: phone}, function (res) {
                if (!res.status)
        ...
        function changeStatus(row) {
            $.post("<?php echo U('Index/changeStatus');?>", {tid: row.id}, function (res) {
                if (!res.status) {
                    $.ligerDialog.error(res.msg);
                } else {
