...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            <?php if(is_array($sights)): foreach($sights as $key=>$sight): ?>CustomersData.Rows.push({
                        "id": "<?php echo ($sight["id"]); ?>",
                        "name": "<?php echo ($sight["name"]); ?>"
        ...
        function itemclick(item) {
            if (item.icon === 'add') {
                $.ligerDialog.prompt('添加', '景点名称', function (yes, value) {
                    if (yes && value !== "") {
                        $.post("<?php echo U('System/sightAdd');?>", {"sight_name": value}, function (res) {
                            if (!res.status)
            ...
            if (item.icon === 'modify') {
                ...
                $.ligerDialog.prompt('修改景点名称', row.name, function (yes, value) {
                    if (yes && value !== "") {
                        $.post("<?php echo U('System/sightEdit');?>", {"id": row.id, "sight_name": value}, function (res) {
                            if (!res.status)
                                $.ligerDialog.error(res.msg);
                            else {
            ...
            if (item.icon === 'delete') {
                ...
                $.ligerDialog.confirm('确认删除？', '删除', function (yes) {
                    if (yes)
                        $.post("<?php echo U('System/sightDel');?>", {"id": row.id}, function (res) {
                            if (!res.status)
                                $.ligerDialog.error(res.msg);
                            else {
