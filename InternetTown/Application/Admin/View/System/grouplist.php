...
<head>
    ...
    <script type="text/javascript">
        ...
        <foreach name = "admingroups" item = "admingroup" >
        ...
        function itemclick(item) {
            if (item.icon === 'add') {
                $.ligerDialog.prompt('添加分组', '分组名称', function (yes, value) {
                    if (yes && value !== "")
                        $.post("{:U('System/groupAdd')}", {group_name: value}, function (res) {
                            if (!res.status) {
            ...
            if (item.icon === 'modify') {
                ...
                $.ligerDialog.prompt('修改分组', row.name, function (yes, value) {
                    if (yes && value !== "")
                        $.post("{:U('System/groupEdit')}", {gid:row.id, group_name:value}, function(res) {
                            if (!res.status) {
                                $.ligerDialog.error(res.msg);
                            } else {
            ...
            if (item.icon === 'delete') {
                ...
                $.ligerDialog.confirm('确认删除？', '删除', function (yes) {
                    if (yes)
                        $.post("{:U('System/groupDel')}", {"gid": row.id}, function (res) {
                            if (!res.status) {
                                $.ligerDialog.error(res.msg);
                            } else {
