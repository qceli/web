...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            <foreach name="tickets" item="ticket">
                    CustomersData.Rows.push({
                        "id":"{$ticket.id}",
                        "billcode":"{$ticket.billcode}",
                        "ticket_name":"{$ticket.ticket_name}",
                        "warning":{$ticket.warning},
                        "sight_info":{$ticket.sight_info},
                        "ticket_price":"{$ticket.ticket_price}",
                        "add_time":"{$ticket.add_time|date='Y-m-d H:i:s', ###}"
                    });
            </foreach>
        ...
        function itemclick(item) {
            ...
            if (item.icon === 'delete') {
                ...
                $.ligerDialog.confirm('确定要删除？',function (yes) {
                    if (yes) {
                        $.post("{:U('Ticket/del')}", {id:row.id}, function (res) {
                            if (!res.status) {
